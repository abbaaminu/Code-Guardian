...
export function ConnectRepositoryPanel({
  ...
  const fetchGitHubRepos = async () => {
    ...
    try {
      ...
      if (parts.length === 0) {
        throw new Error(
          "Please enter a valid GitHub username or repository URL",
        );
      }
      ...
      if (parts.length >= 2) {
        // Case 1: Specific repository URL or path entered (e.g., octocat/Hello-World)
        const owner = parts[0];
        ...
        // Case 2: Only username or organization provided (e.g., octocat)
        const owner = parts[0];
        const response = await fetch(
          `https://api.github.com/users/${owner}/repos?sort=updated&per_page=15`,
        );
  ...
  const handleScanRepo = async (repo: any) => {
    ...
    try {
      ...
      const sourceCode = await fetchRepoSourceCode(
      ...
      setError(
        err instanceof Error
          ? err.message
          : `Failed to read files from ${repo.full_name}`,
      );
    } finally {
      ...
      <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
        ...
        {repos.length === 0 && !loading && !error && (
          <p className="text-xs text-slate-400 text-center py-4">
            Enter a username or repository URL and click "Fetch Repos".
          </p>
        )}
      </div>
    </div>
