...
function tokenize(line: string): Token[] {
  ...
  while ((m = re.exec(line)) !== null) {
    ...
    if (s.startsWith("//") || s.startsWith("#") || s.startsWith("/*"))
      c = "text-[oklch(0.62_0.03_160)] italic";
    else if (s.startsWith('"') || s.startsWith("'") || s.startsWith("`"))
      c = "text-[oklch(0.82_0.14_45)]";
    else if (/^0x[0-9a-fA-F]+$/.test(s) || /^\d/.test(s))
      c = "text-[oklch(0.78_0.16_195)]";
...
function sevBg(sev: Severity) {
  switch (sev) {
    case "critical":
      return "bg-critical/20 border-l-2 border-critical shadow-[inset_0_0_24px_-6px_var(--critical)]";
    case "high":
      return "bg-high/15 border-l-2 border-high shadow-[inset_0_0_18px_-8px_var(--high)]";
    case "medium":
      return "bg-medium/15 border-l-2 border-medium";
    case "low":
      return "bg-low/15 border-l-2 border-low";
  }
}
...
export function CodeVault({
  ...
  useEffect(() => {
    ...
    if (virtualize) {
      virtualizer.scrollToIndex(Math.max(0, activeRange.start - 1), {
        align: "center",
        behavior: "smooth",
      });
    } else {
